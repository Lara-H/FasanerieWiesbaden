...
fun GoatScreen(isCorrect: Boolean, imagePath: Int, onCorrectClick: () -> Unit, onFalseClick: () -> Unit) {
    Scaffold(
        topBar = { TopBar(text="Ziegenwiese", isMainNav = false) },
    ) {
    }
    ...
    Column(modifier = Modifier
        ...
        Column(modifier = Modifier
            ...
            Text(
                ...
                text = "Schau dir die Bilder der Ziegenrassen auf dem Infoschild genau an.",
            )
            Text(
                modifier = Modifier
                    .padding(bottom = MaterialTheme.spacing.medium),
                text = "Ist das eine Ziege?",
            )
        }
        Column(modifier = Modifier
            .padding(all = MaterialTheme.spacing.medium),
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                Button(modifier = Modifier.weight(1f).padding(end = MaterialTheme.spacing.medium/2),
                    onClick = { handleClick(true, isCorrect, onCorrectClick, onFalseClick) }) {
                    Icon(Icons.Default.Check, "Icon", tint = Color.White)
                }
                Button(modifier = Modifier.weight(1f).padding(start = MaterialTheme.spacing.medium/2),
